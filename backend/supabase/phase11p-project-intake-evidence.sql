-- Project creation attachments are intake evidence, separate from the official
-- Document Repository. Apply manually through the Supabase SQL editor.
begin;

create table if not exists public.project_intake_attachments (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  kind text not null check (kind in ('MOM', 'PHOTO', 'DOCUMENT')),
  original_filename text not null,
  storage_path text not null unique,
  mime_type text not null,
  size_bytes bigint not null check (size_bytes >= 0),
  -- Immutable audit identifier without a user FK, consistent with Phase 11I/11L.
  created_by uuid not null,
  created_at timestamptz not null default timezone('utc'::text, now()),
  constraint project_intake_attachments_filename_check
    check (char_length(btrim(original_filename)) > 0),
  constraint project_intake_attachments_storage_path_check
    check (char_length(btrim(storage_path)) > 0),
  constraint project_intake_attachments_mime_type_check
    check (char_length(btrim(mime_type)) > 0)
);

create index if not exists project_intake_attachments_project_created_idx
  on public.project_intake_attachments(project_id, created_at asc);

create unique index if not exists project_intake_attachments_one_mom_per_project_idx
  on public.project_intake_attachments(project_id)
  where kind = 'MOM';

alter table public.project_intake_attachments enable row level security;
drop policy if exists project_intake_attachments_service_role_only
  on public.project_intake_attachments;
create policy project_intake_attachments_service_role_only
  on public.project_intake_attachments
  for all to service_role using (true) with check (true);

revoke all on table public.project_intake_attachments from public, anon, authenticated;
grant select, insert, update, delete on table public.project_intake_attachments to service_role;

-- Replace the latest project-deletion RPC so exact intake object paths are
-- captured before the project row and its intake metadata are removed.
...
-- select project_id, kind, original_filename, mime_type, size_bytes, created_at
-- from public.project_intake_attachments
